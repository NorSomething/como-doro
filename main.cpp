...
void pomo::draw_main_tui(int choice) {
  ...
  mvwprintw(heading_win, 6,1, "Pomo Session has ended.");
  wrefresh(heading_win);
...
void pomo::count_down(int sesh_len_min, WINDOW *heading_win) {
  ...
  mvwprintw(heading_win, 5, 1, "Press q to cancel pomo : ");
  
  nodelay(heading_win, TRUE); //makes input non blocking for other functions of heading_win
  ...
  while (chrono::steady_clock::now() - start < duration) {
    ...
    int ch = wgetch(heading_win); //wgetnstr is blocking, hence not used

    if (ch == 'q' || ch == 'Q') {
      mvwprintw(heading_win, 6,1, "Pomo Doro Cancelled");
      wrefresh(heading_win);
